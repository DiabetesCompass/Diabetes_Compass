...
- (void) calculateHa1c:(BGReading*) bgReading
{
//This method calculates a weighted average HA1c using a weighted average BG over the past 100 days of recorded BG values.
    //Check to see if there is only 1 object in bgArray

    
    //1 -- Retreive all blood glucose readings from the past 100 days. Backwards from the BGReading of interest.
    ...
    BGReading* previousReading = nil;
    int bigIndex = 0;
    float ramp = 1.00000000000;
    float delta = 0.000069444;
    float sum =0.00000000000;
    float sumRamp = 0.00000000000;
    float twBGAve = 0.00000000000;
    float twHA1c = 0.00000000000;
    for (BGReading* reading in fetchedReadings) {
//        NSLog(@"The current reading's timestamp:%@, quantity:%@", reading.timeStamp, reading.quantity);
//        NSLog(@"The previous reading's timestamp:%@, quantity:%@", previousReading.timeStamp, previousReading.quantity);
