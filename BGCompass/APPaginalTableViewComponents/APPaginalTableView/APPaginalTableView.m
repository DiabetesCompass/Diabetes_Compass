...
- (NSUInteger)countExpandableElements
{
    NSUInteger count = 0;
    for (id element in _elementViews) {
        if ([((APPaginalTableViewElement*)element).collapsedView isKindOfClass:[CollapsedTrendView class]]) {
            if (((CollapsedTrendView*)((APPaginalTableViewElement*)element).collapsedView).expandable) {
                count++;
            }
        } else {
            count++;
...
- (void)onPinchBegan:(UIPinchGestureRecognizer*)pinchRecognizer
{
    ...
    if (indexPath != nil) {
        ...
        if ([view.collapsedView isKindOfClass:[CollapsedTrendView class]] && ((CollapsedTrendView*)view.collapsedView).expandable) {
...
- (NSInteger)numberOfPagesInPaginalContainerView:(APPaginalContainerView *)paginalContainerView
{
    ...
    return [self countExpandableElements];
}
...
{
    APPaginalTableViewElement *view = _elementViews[index];
    if ([view.collapsedView isKindOfClass:[CollapsedTrendView class]]) {
        if (((CollapsedTrendView*)view.collapsedView).expandable) {
...
- (void)scrollToContentBoundsCellAtIndex:(NSInteger)indexSelected animated:(BOOL)animated
{
    ...
    if (visibleTableHeight > heightOfContent) {
        ...
        if (isnan(percentOfMaxOffset)) {
            percentOfMaxOffset = 0.f;
        }
